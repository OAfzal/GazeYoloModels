...
def _create(
    ...
    try:
        ...
        if pretrained and channels == 3 and classes == 80:
            ...
            if pretrained:
                ckpt = torch.load(
                    attempt_download(path, "OAfzal/GazeYoloModels"), map_location=device
                )  # load
                csd = (
...
def gaze_model(
    pretrained=True, channels=3, classes=1, autoshape=True, verbose=True, device=None
):
    # YOLOv3 model https://github.com/ultralytics/yolov3
    return _create(
        "gaze_model", pretrained, channels, classes, autoshape, verbose, device
    )
...
def hand_model(
    ...
    # YOLOv3 model https://github.com/ultralytics/yolov3
    return _create(
        "hand_model", pretrained, channels, classes, autoshape, verbose, device
    )
